...
- `shared/llm_client.py` — single OpenAI client (gpt-5.5). All services import from here. **Never re-add Anthropic support without explicit user request.**
- `shared/indications.yaml` — controlled vocabulary of indications per intervention class. Read by `/api/configs/indications` and stamped on every output. **Indications are NOT owned by any service config.**

## The 4 primitives (required for every tool run)

Every request stamps these 4 fields on every output. All 4 are required at the UI level (`isHeaderComplete` enforces).
...
- Folders use **action names**: `services/chunker/`, `services/benchmarker/`, `services/reviewer/`.
- Data units stay as their nouns: `Claim`, `ClaimsStore`, `data/claims/`, `ContentBlock`.
...
- Don't pass `provider` / `model` form fields per request. The LLM client is fixed (OpenAI gpt-5.5 today); swap by editing `shared/llm_client.py`, not by adding form fields.
...
```
shared/llm_client.py             OpenAI client
