...
- rank orders every unit that needs work across the whole document: sort by rank to get "what to fix first". Do not re-rank. The order is the verdict's own place in the vocabulary above, which is declared worst-first, then the rubric author's own section and variable sequence - there is no weighting to apply.
- document_findings[]: conflicts spanning sections, which no single unit owns. verdict is always section_conflict and both names are null; the sections involved are read back from cited_block_ids against sections[].mapped_block_ids.
...
EXPERT_LEGEND = """This is an EXPERT result: one stage gate's question bank triaged against a set of product-development documents. Expert does not answer the questions and renders no verdict on the documents - it reports which of them the supplied material answers and which it does not. Shape:
