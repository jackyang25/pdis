...
- What a reader sees these called. The field names below are the data; the interface renders
  them as: matches -> "External evidence" (Conflicts / Supports / Adds context / Unrelated),
  assessments -> "Grounding", conformity -> "Measurable targets", precedents -> "Precedent",
  variables[].document_target -> "Target stated in document". Use the reader's words when
  answering a person and the field name only when naming where something came from.
...
- verdict is one of: specified (the rubric asks for this and the document supplies it usably), not_present (nothing is there), placeholder (a token such as <<TBD>> sits where the value belongs), insufficient (content is present but part of what the requirement asks for is absent), vague (covers the requirement but is unusable as stated - unmeasurable or too general to act on), section_conflict (two sections disagree), not_applicable (the rubric itself marks the unit optional and the document omits it, so it is NOT a shortfall).
- insufficient and vague are not degrees of each other: coverage first, then usability. If any part of what the requirement asks for is absent, it is insufficient. Only if the content covers the whole requirement and is still unusable is it vague.
- ONE AXIS, and there is no second one. There is no severity field, no level, no separate unit status, no letter grade and no overall score. A count of anything but specified and not_applicable is a count of fixable items.
- statement says what is wrong, in one sentence. It is empty exactly when the verdict is specified or not_applicable, because there is nothing wrong to describe. There is no recommendation field: a reader who knows what is missing knows to add it.
- cited_block_ids is empty exactly when the verdict is not_present or not_applicable, because absence cites nothing. Every other verdict cites the passage it was read from, so it can be checked against the document.
- rank orders units that need work within their review: sort by rank to get "what to fix first" in that rubric. Do not re-rank or compare ranks between rubrics. The order is the verdict's place in the vocabulary, then the rubric author's own sequence.
- document_findings[]: one document-wide consistency check, not repeated per rubric. Conflicts concern the document's own statements, not differences between rubric requirements. verdict is section_conflict and both names are null; citations identify the document passages.
- consistency_status and assessment_status describe whether the run completed. They are process facts, never verdicts: a failed check means the document was not checked, which is not the same as a document with nothing wrong."""
...
- findings[]: every requirement read out of a reference document, each judged exactly once. This is the denominator as well as the content, so a count taken from it is safe to quote. Each finding carries:
    requirement: the bar, in the reference document's own words, with reference_spans quoting the exact lines that state it. Aligner did not decide what matters; it read this out of the document. Compound sentences were split, so one requirement is one fact.
...
- state is answered | partly_answered | not_found | not_applicable, and only the first three come from a model. They are ordered by how much of the question is closed, because these questions are compound - most ask three to five things in one sentence and are judged clause by clause:
    ...
    partly_answered = some parts are and some are not. `statement` and `missing` divide the question between them: `statement` is the part the documents DO cover and `missing` names what is still not stated. Report both halves rather than one - a partial reported as only its gap reads as an unanswered question, and reported as only its statement reads as a finished one. `missing` is the actionable half: it is what a reader takes back to whoever wrote the document, so always quote or paraphrase it;
