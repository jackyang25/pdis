...
- conformity[]: per quantitative variable, a source-quoted comparator cohort tested against an exact block-cited target_quote. Each canonical quantitative target records one field owner and one required comparison-axis profile; applicability is not reinterpreted per source. measurements[] contains only candidates whose required axes were admitted deterministically; excluded_measurements[] retains exact candidates and rejection reasons. Source records are deduplicated before calculation. Benchmark minimum/maximum/mean/median/quartiles/standard deviation and target/ambition percentiles describe this selected cohort only. target_meeting_count/rate is the literal observed share, not a confidence interval or forecast probability; descriptive source labels do not weight it. `legacy_unverified` means the imported file predates this provenance contract and requires a rerun.
