...
- conformity[]: per quantitative variable, a source-quoted comparator cohort tested against an exact block-cited target_quote. measurements[] contains only candidates whose endpoint, population, intervention, regimen, time_horizon, and statistic were labeled same/compatible/not_applicable and then admitted deterministically; excluded_measurements[] retains grounded candidates and explicit rejection reasons. Source records are deduplicated before calculation. Benchmark minimum/maximum/mean/median/quartiles/standard deviation and target/ambition percentiles describe this selected cohort only. target_meeting_count/rate is the literal observed share, not a confidence interval or forecast probability; descriptive source labels do not weight it. `legacy_unverified` means the imported file predates this provenance contract and requires a rerun.
