"""Binder: assigns `attribute_ref` to draft claims.
...
actively consumes the AttributeConfig's attribute vocabulary.
"""
...
def bind_claims(
    ...
    *,
    ...
    """Fill `attribute_ref` on each claim.
    ...
    failures (missing ID, unknown attribute_ref) leave that claim's
    attribute_ref as None.
    """
    if not claims:
        return claims
    ...
    for index, claim in enumerate(claims):
        ...
        if isinstance(attribute_ref, str) and attribute_ref in valid_attribute_names:
...
def _build_system_prompt(config: AttributeConfig) -> str:
    ...
    return (
        ...
        f"{config.preamble}\n\n"
        "Each input claim is an atomic, source-backed assertion. Bind each "
        "claim to exactly one attribute from the list below, or leave it unbound.\n\n"
        "ATTRIBUTES (pick from these exact names):\n"
        f"{attribute_lines}\n\n"
        "Rules:\n"
        "  - Choose the single best attribute. If none fits well, omit attribute_ref.\n"
