"""One declaration per model prompt Expert sends, for publication and testing.

The assessor owns its prompt text. This module owns the list of prompts and what
each produces.

Expert sends one prompt, once per queued question, so this single pair is the
...
PROMPT_CATALOG: tuple[CatalogEntry, ...] = (
    CatalogEntry(
        ...
        # Expert has no framing slot. The bank supplies each question's whole text,
