...
DEFAULT_AGE_MONTHS = 24.0            # assumed age when a source has no date
...
def score_conformity(
    ...
    *,
    ...
    target_value, comparator, unit, measurements = extracted
    if not measurements:
        return None

    _attach_weights(measurements, insights)
    return _combine(attribute.name, target_value, comparator, unit, measurements)
...
def _extract_measurements(
    ...
    *,
    ...
    intervention_class: str,
    max_tokens: int,
) -> tuple[float, str, str, list[Measurement]] | None:
    ...
    return float(target), comparator, unit, measurements
...
def _system_prompt(attribute: Attribute, indication: str, intervention_class: str) -> str:
    return (
        ...
        "2. If quantitative, read the document target value, its comparator "
        "(\">=\" when higher is better, \"<=\" when lower is better), and the unit.\n"
        "3. From the web insights, extract each source's reported numeric value for "
        "THIS variable (same unit as the target). Skip insights with no comparable "
        "number. Classify each source into one source_type from this list:\n"
        ...
        '"unit": "%", "measurements": [{"value": 75, "source_type": "rct_phase3", '
        '"url": "https://..."}]}\n'
