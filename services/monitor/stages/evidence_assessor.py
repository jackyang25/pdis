...
def _system_prompt(
    *,
    ...
    return (
        ...
        "2. Judge whether the target is a JUSTIFIED IMPROVEMENT OVER THE CURRENT STANDARD OF CARE for this indication - not merely whether evidence about the variable exists. A target is well-grounded when it beats the standard of care by a margin that modeling and/or clinical evidence supports (higher efficacy, shorter regimen, fewer doses, etc.), or matches an authoritative precedent.\n"
        "3. Weigh evidence by type: a standard-of-care anchor (what current products achieve), modeling that ties the target to a health outcome, study strength (clinical-trial data weighs more than preclinical/animal-model data), and regulatory precedent including national regulators.\n\n"
        "Strength enum:\n"
        "- well_grounded: the target is a clear improvement over the standard of care, supported by strong modeling or clinical evidence (or matches authoritative precedent).\n"
        "- partial: the target is plausibly better than the standard of care, but the supporting evidence has gaps.\n"
        "- thin: evidence is sparse, indirect, or preclinical-only; the target is weakly benchmarked against the standard of care.\n"
        "- unsupported: the target is arbitrary or not benchmarked against any standard of care.\n"
