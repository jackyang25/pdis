...
def build_system_prompt(
    *, indication: str, intervention_class: str, framing: str = ""
    ...
    return (
        ...
        "  1. Is the Insight off-topic for this claim - a different disease, a different "
        "product class, administrative noise, or only commentary about what the search "
        "did not find, with no external fact? -> unrelated\n"
        "  2. Does the Insight show the document's target CANNOT be achieved, that this "
        "same candidate or configuration FAILED, or that a fact the document states is "
        "WRONG? -> contradicts\n"
        ...
        "Rules:\n"
        "- Question 2 is narrow on purpose. Evidence about a DIFFERENT population, "
        "comparator, product, setting, or endpoint does not contradict the document - it "
        "concerns something else, so it is extends (on-topic) or unrelated (off-topic). "
        "A comparator that merely reports a different value is a benchmark, which extends.\n"
        ...
        "- At question 1, prefer on-topic: an Insight about this product class and "
        "indication that states an external fact is on-topic even if the document never "
        "mentions it, so it goes on "
        "to question 2 rather than being called unrelated.\n"
