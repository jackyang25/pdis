...
const TOPICS: Record<ExpertSignalTopic, SignalTopic> = {
  state: {
    ...
    detail:
      "These questions are compound, with many asking two or three things in one sentence, so they are judged clause by clause. Answered means every part is answered. Partly answered means some parts are and some are not, and the two halves are shown separately: the sentence says what the documents DO cover, and the line beneath it names what is still not stated. That second line is the specific thing to ask the grantee for, and it is the most actionable output here. If it could not be named specifically, the question is reported as answered rather than as a partial nobody can act on. A list in brackets inside a question tells the model what counts as addressing the term in front of it; it is not read as a checklist of separate demands, because it is the source's way of saying what the term means. Not found means the question was read against everything you supplied and nothing addressed it. Nothing more than that. It is not a judgment about whose fault it is, and not a claim that one of your documents should have contained it: many of these questions ask about operational facts or matters of judgment that no profile or plan would ever carry, and this tool cannot tell those apart from a real omission. What it can tell you is which discipline owns the question. Not applicable means the question's own text restricts it to a different intervention class, so no model read it and it is not a shortfall of any kind. Partly answered is not a progress bar and there is no score: 40 partial answers is not 'half done'.",
  },
  source: {
    promptRef: { tool: "expert", stage: "triage" },
    title: "Source",
  ...
  requirement: {
    ...
    detail:
      "The source states one of these for every question, and it is what makes an open question actionable. A required question the documents do not answer is what holds a gate up. An anticipatory one is early warning: the gate expects thinking to be under way, not a finished answer, so an unanswered anticipatory question is a prompt for the next conversation rather than a shortfall. Only required questions carry the badge, because labelling both would put a mark on every row, which marks nothing. It changes nothing about how a question is read: the model is never told which kind it is, because a model told a question is only anticipatory would read the material less carefully for it.",
  },
  denominator: {
    title: "The count",
